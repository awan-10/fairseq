    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def forward(
        ...
        Returns:
            encoded output of shape `(seq_len, batch, embed_dim)`
        """
        print(f"normalize_before in fairseq = {self.normalize_before}")
        print(f"x norm in fairseq TransformerDecoderLayerBase forward = {x.norm()}")
        ...
        if self.normalize_before:
            x = self.self_attn_layer_norm(x)
            print(f"norm of x changes in normalize before = {x.norm()}")
        ...
        if self.cross_self_attention and not (
            ...
            y = x
        print(f"x norm = {x.norm()} and y norm = {y.norm()}")
        ...
        print(f"attn norm = {x.norm()}, x.shape = {x.shape}")#x = {x}")
        ...
        residual = x
        if self.normalize_before:
            print(f"x after norm {x.norm()}")
            x = self.final_layer_norm(x)

        print(f"norm of x b4 fc1 = {x.norm()}")
        #exit(0)
        print(f"self.activation fn = {self.activation_fn}")
        x = self.activation_fn(self.fc1(x))
        x = self.activation_dropout_module(x)
        if self.ffn_layernorm is not None:
            x = self.ffn_layernorm(x)
        x = self.fc2(x)
        print(f"fairseq mlp out = {x.norm()}")
